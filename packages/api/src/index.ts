...
// Fetch Are.na channel contents and optionally resolve stream URLs via Radio Browser API
// Query: slug = channel slug (e.g. "online-radios-zlvblzsstly" or "chia/online-radios-zlvblzsstly")
app.get("/api/are-na-channel", async (req, res) => {
  const rawSlug = typeof req.query.slug === "string" ? req.query.slug.trim() : "";
  if (!rawSlug) return res.status(400).json({ error: "Missing slug" });
  const slug = rawSlug.replace(/^\/+|\/+$/g, "").replace(/^https?:\/\/[^/]+\//i, "") || rawSlug;
  try {
    const controller = new AbortController();
    const t = setTimeout(() => controller.abort(), 12000);
    const url = `https://api.are.na/v2/channels/${encodeURIComponent(slug)}/contents`;
    const response = await fetch(url, {
      method: "GET",
      signal: controller.signal,
    ...
    clearTimeout(t);
    if (!response.ok) {
      return res.status(response.status).json({ error: "Are.na channel not found", stations: [] });
    }
    const data = (await response.json()) as { contents?: Array<{ class?: string; source?: { url?: string } | string | null; title?: string | null; description?: string | null; image?: { display?: { url?: string } } | null }> };
    const contents = data.contents || [];
    const blocks = contents.filter((b) => b && (b.class === "Block" || !b.class) && b.source != null);
    const RADIO_BROWSER = "https://nl1.api.radio-browser.info";
    const stations: Array<{ name: string; description: string; websiteUrl: string; streamUrl: string | null; logoUrl: string }> = [];

    for (const block of blocks) {
      const src = block.source;
      const websiteUrl = (typeof src === "string" ? src : (src as { url?: string }).url || "").trim();
      if (!websiteUrl || (!websiteUrl.startsWith("http://") && !websiteUrl.startsWith("https://"))) continue;
      const name = (block.title || new URL(websiteUrl).hostname.replace(/^www\./, "") || "Radio").trim();
      const description = (block.description || "").trim().slice(0, 200) || "Online radio from Are.na.";
      const image = block.image as { display?: { url?: string } } | undefined;
      const logoUrl = image?.display?.url?.trim() || `https://www.google.com/s2/favicons?domain=${encodeURIComponent(new URL(websiteUrl).hostname)}&sz=128`;

      let streamUrl: string | null = null;
      try {
        const searchName = name.replace(/\s*[|\-–—].*$/, "").trim().slice(0, 50);
        const rbCtrl = new AbortController();
        const rbT = setTimeout(() => rbCtrl.abort(), 5000);
        const rbRes = await fetch(
          `${RADIO_BROWSER}/json/stations/search?name=${encodeURIComponent(searchName)}&limit=5`,
          { signal: rbCtrl.signal, headers: { "User-Agent": "LAF/1.0" } }
        );
        clearTimeout(rbT);
        if (rbRes.ok) {
          const arr = (await rbRes.json()) as Array<{ url_resolved?: string; homepage?: string; favicon?: string }>;
          const host = new URL(websiteUrl).hostname.toLowerCase();
          const match = arr.find((s) => s.url_resolved && (s.homepage || "").toLowerCase().includes(host) || host.includes((new URL(s.homepage || "http://x").hostname).toLowerCase()));
          if (match?.url_resolved) streamUrl = match.url_resolved;
          else if (arr[0]?.url_resolved && (arr[0].homepage || "").toLowerCase().includes(host)) streamUrl = arr[0].url_resolved;
        }
      } catch {
        // leave streamUrl null
      }

      stations.push({ name, description, websiteUrl, streamUrl, logoUrl });
    }

    res.json({ slug, stations });
  } catch (err: unknown) {
    console.error("[are-na-channel]", err);
    res.status(500).json({ error: "Failed to fetch Are.na channel", stations: [] });
  }
});
